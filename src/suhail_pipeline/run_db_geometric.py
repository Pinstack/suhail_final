...
def main(
    ...
    while True:
        ...
        # Aggregate decoded data per layer (mark processed only after successful persist)
        ...
        def persist_layers() -> None:
            for layer in settings.layers_to_process:
                ...
                # Keyless multi-row layers (e.g. dimensions): idempotent tile-scoped
                # accumulate instead of the per-batch replace that would keep only the
                # last batch. Deletes this batch's tiles' rows, then appends.
                if layer in TILE_SCOPED_LAYERS:
                    persister.write_tile_scoped(
                        gdf, layer, table_name, chunksize=settings.db_chunk_size
                    )
                    continue
