...
class Parcel(Base):
    ...
    # Inline market time-series now emitted on the parcels MVT layer (2026-07).
    transaction_price_1w = Column(Float)
    transaction_price_1m = Column(Float)
    transaction_price_6m = Column(Float)
    transaction_price_12m = Column(Float)
    price_of_meter_1w = Column(Float)
    price_of_meter_1m = Column(Float)
    price_of_meter_6m = Column(Float)
    price_of_meter_12m = Column(Float)
    transactions_count_1w = Column(BigInteger)
    transactions_count_1m = Column(BigInteger)
    transactions_count_6m = Column(BigInteger)
    transactions_count_12m = Column(BigInteger)
    transaction_date_1w = Column(DateTime)
    transaction_date_1m = Column(DateTime)
...
class Transaction(Base):
    ...
    area = Column(Float)
    # Materially-useful attributes now promoted out of raw_data into queryable columns.
    transaction_type = Column(String)
    property_type = Column(String)
    metrics_type = Column(String)
    land_use_group = Column(String)
    land_use_detailed = Column(String)
    selling_type = Column(String)
    transaction_source = Column(String)
    total_area = Column(Float)
    subdivision_id = Column(BigInteger)
    neighborhood_id = Column(BigInteger)
    is_low_value_transaction = Column(Boolean)
...
class Neighborhood(Base):
    ...
    # Inline market time-series now emitted on the neighborhoods MVT layer (2026-07).
    transaction_price_1w = Column(Float)
    transaction_price_1m = Column(Float)
    transaction_price_6m = Column(Float)
    transaction_price_12m = Column(Float)
...
class Subdivision(Base):
    ...
    geometry = Column(Geometry('GEOMETRY', srid=4326))
    subdivision_no = Column(String)
    subdivision_name_ar = Column(String)
    neighborhood_id = Column(BigInteger)
    region_id = Column(BigInteger)
    shape_area = Column(Float)
    transaction_price = Column(Float)
    ...
    # Inline market time-series now emitted on the subdivisions MVT layer (2026-07).
    transaction_price_1w = Column(Float)
    transaction_price_1m = Column(Float)
    transaction_price_6m = Column(Float)
    transaction_price_12m = Column(Float)
...
class ParcelsCentroids(Base):
    ...
    transaction_price = Column(Float)
    price_of_meter = Column(Float)
    transactions_count = Column(BigInteger)
    transaction_price_1w = Column(Float)
    transaction_price_1m = Column(Float)
    transaction_price_6m = Column(Float)
    transaction_price_12m = Column(Float)
...
class BusLines(Base):
    ...
    # No stable unique key in the tile; written in replace mode. Native tile fields.
...
class QIPopulationMetrics(Base):
    ...
    region_id = Column(BigInteger)
    # Values are colour-coded metric strings (hex) in the current tile schema.
    population_density = Column(String)
    rent_apartment = Column(String)
    rent_villa = Column(String)
    rent_shop = Column(String)
    rent_office = Column(String)
    purchasing_power = Column(String)
    weighted_median_income_monthly = Column(String)
    poi_count = Column(String)
...
class NonSaudiOwnershipZones(Base):
    __tablename__ = 'non_saudi_ownership_zones'
    ...
    region_id = Column(BigInteger)
    province_id = Column(BigInteger)

class Dimensions(Base):
    __tablename__ = 'dimensions'
    # Per-parcel edge measurements: many rows per parcel (one point per boundary edge),
    # no natural unique key -> persisted via tile-scoped delete+append keyed on source_tile.
    row_id = Column(BigInteger, primary_key=True, autoincrement=True)
    parcel_objectid = Column(BigInteger)
    geometry = Column(Geometry('POINT', srid=4326))
    length_m = Column(Float)
    azimuth = Column(Float)
    province_id = Column(BigInteger)
    source_tile = Column(String)

class BuildingDetection(Base):
    __tablename__ = 'building_detection'
    # AI-detected building footprints/classification, year-stamped. Keyless in the
    # source -> deterministic synthetic bd_id (see SYNTHETIC_PK_CONFIG) for upsert.
    bd_id = Column(BigInteger, primary_key=True, autoincrement=False)
    geometry = Column(Geometry('GEOMETRY', srid=4326))
    class_pred = Column(String)
    prediction_year = Column(BigInteger)
    region_id = Column(BigInteger)
    source_tile = Column(String)
