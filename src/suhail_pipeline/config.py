...
from suhail_pipeline.persistence.db import load_provinces_from_db
...
ARABIC_COLUMN_MAP = {
    ...
    "name_aname": "name_ar",
    # Municipality name on the parcels layer (source field: municipality_aname).
    # Without this entry municipality_ar was always NULL.
    "municipalityaname": "municipality_ar",
    "municipality_aname": "municipality_ar",
    # Subdivision Arabic name now emitted on the subdivisions layer.
    "subdivision_name_ar": "subdivision_name_ar",
    # Add any other discovered or future Arabic columns here
}

# Time-series market fields now emitted inline on parcels/neighborhoods/subdivisions
# tile layers. Mapped to 'first' so they survive the PostGIS dissolve/stitch step.
_MARKET_TIMESERIES_FIRST = {
    field: "first"
    for field in (
        "transaction_price_1w", "transaction_price_1m",
        "transaction_price_6m", "transaction_price_12m",
        "price_of_meter_1w", "price_of_meter_1m",
        "price_of_meter_6m", "price_of_meter_12m",
        "transactions_count_1w", "transactions_count_1m",
        "transactions_count_6m", "transactions_count_12m",
        "transaction_date_1w", "transaction_date_1m",
        "transaction_date_6m", "transaction_date_12m",
    )
}
...
class Settings(BaseSettings):
    ...
    id_column_per_layer: Dict[str, str] = Field(
        default_factory=lambda: {
            ...
            # NB: `dimensions` is intentionally NOT keyed here — it has many rows per
            # parcel (one per edge), so keying on parcel_objectid would collapse it to
            # one edge. It uses the tile-scoped write (postgis_persister.TILE_SCOPED_LAYERS).
            "metro_stations": "station_code",
            "riyadh_bus_stations": "station_code",
            "qi_population_metrics": "grid_id",
            "qi_stripes": "strip_id",
            "non_saudi_ownership_zones": "id",
            # Synthetic deterministic key (postgis_persister.SYNTHETIC_PK_CONFIG).
            "building_detection": "bd_id",
        },
    ...
    table_name_mapping: Dict[str, str] = Field(
        default_factory=lambda: {
            ...
            "non_saudi_ownership_zones": "non_saudi_ownership_zones",
            # Documented-but-not-yet-enabled new layers map to their own tables:
